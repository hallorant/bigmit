...
; Dynamically detects the mod and, if found, animates a horizontal line.
  org $4200
screen         equ  $3c00
blank_line     dc   64,$80
top_line       dc   64,$83
...
detect_m1_vblank:
  ld hl,1350
  ld bc,-1
  ; A Model 1 will not have a 0 in bit 0 of in $ff without the VBLANK mod.
  ; We want to observe for no less than 1/60th of a second, or 29566 t-states.
  ; The loop below is 44 t-states * 1354 loops = 59400 (~1/30th of a second)
  ; which should be long enought.
_vblank_search_loop:
  in a,($ff)
  bit 0,a
  jr z,_vblank_found
...
; Waits for the start of the next VBLANK (even if one is ongoing).
wait_for_vblank_start macro ?wait_already_in_vblank,?wait_not_in_vblank
?wait_already_in_vblank:
  in a,($ff)
  bit 0,a
  jr nz,?wait_already_in_vblank
?wait_not_in_vblank:
  in a,($ff)
  bit 0,a
...
vblank_found:
  ...
  wait_for_vblank_start ; first time only
smooth_animation:
  irpc row,0123456789
